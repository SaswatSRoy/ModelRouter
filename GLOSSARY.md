...
| **Routing Strategy** | A pluggable algorithm that ranks Scored Candidates to produce the `ExecutionPlan`. Implementations include `CheapestViableStrategy`, `LowestLatencyStrategy`, and others. Selected by the Execution Planner based on the resolved Routing Policy. |
